...
static HRESULT vkd3d_late_compile_shader_stages(struct d3d12_pipeline_state *state)
{
    ...
    for (i = 0; i < graphics->stage_count; i++)
    {
        graphics->identifiers[i].pIdentifier = NULL;
        graphics->identifiers[i].identifierSize = 0;
        ...
        const D3D12_SHADER_BYTECODE *code)
{
    ...
    vk_cache = state->vk_pso_cache ? state->vk_pso_cache : device->global_pipeline_cache,
    ...
    if (state->compute.identifier.identifierSize == 0)
    {
    ...
    if (FAILED(hr = vkd3d_setup_shader_stage(state, device,
            ...
            &required_subgroup_size_info, &state->compute.identifier,
            spirv_code)))
        return hr;
    ...
    {
        state->compute.identifier.identifierSize = sizeof(state->compute.identifier_buffer);
        VK_CALL(vkGetShaderModuleIdentifierEXT(device->vk_device, pipeline_info.stage.module,
                &state->compute.identifier.identifierSize, state->compute.identifier_buffer));
        state->compute.identifier.pIdentifier = state->compute.identifier_buffer;
    }
    ...
    {
        pipeline_info.flags &= ~VK_PIPELINE_CREATE_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT_EXT;
        state->compute.identifier.identifierSize = 0;
        state->compute.identifier.pIdentifier = NULL;
        ...
        const struct d3d12_cached_pipeline_state *cached_pso)
{
    ...
    vkd3d_load_spirv_from_cached_state(device, cached_pso,
            VK_SHADER_STAGE_COMPUTE_BIT, &state->compute.code, &state->compute.identifier);
        ...
        const struct d3d12_cached_pipeline_state *cached_pso)
{
    ...
    for (i = 0; i < graphics->stage_count; i++)
    {
        if (FAILED(vkd3d_load_spirv_from_cached_state(device, cached_pso,
                graphics->cached_desc.bytecode_stages[i], &graphics->code[i], &graphics->identifiers[i])))
        {
            for (j = 0; j < i; j++)
            {
                if (vkd3d_config_flags & VKD3D_CONFIG_FLAG_PIPELINE_LIBRARY_LOG)
        ...
        const struct d3d12_pipeline_state_desc *desc)
{
    ...
    for (i = 0; i < graphics->stage_count; i++)
    {
        if (state->graphics.identifiers[i].identifierSize == 0)
        {
            if (FAILED(hr = vkd3d_compile_shader_stage(state, device,
                    graphics->cached_desc.bytecode_stages[i],
                    &graphics->cached_desc.bytecode[i], &graphics->code[i])))
        ...
        struct d3d12_device *device)
{
    ...
    for (i = 0; i < graphics->stage_count; i++)
    {
        ...
        if (graphics->stages[i].module != VK_NULL_HANDLE &&
                device->device_info.shader_module_identifier_features.shaderModuleIdentifier)
        {
            state->graphics.identifiers[i].identifierSize = sizeof(state->graphics.identifier_buffers[i]);
            VK_CALL(vkGetShaderModuleIdentifierEXT(device->vk_device, graphics->stages[i].module,
                    &graphics->identifiers[i].identifierSize,
                    state->graphics.identifier_buffers[i]));
            state->graphics.identifiers[i].pIdentifier = state->graphics.identifier_buffers[i];
        }
    }
}
