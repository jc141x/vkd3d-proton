...
static void vkd3d_shader_code_serialize_identifier(struct d3d12_pipeline_library *pipeline_library,
        ...
        const VkPipelineShaderStageModuleIdentifierCreateInfoEXT *identifier, VkShaderStageFlagBits stage,
        struct vkd3d_pipeline_blob_chunk **inout_chunk)
{
    struct vkd3d_pipeline_blob_chunk *chunk = *inout_chunk;
    struct vkd3d_pipeline_blob_chunk_shader_meta *meta;
    ...
    memcpy(chunk->data, identifier->pIdentifier, chunk->size);
    chunk = finish_and_iterate_blob_chunk(chunk);
        ...
        struct d3d12_device *device)
{
    ...
    if (!(vkd3d_config_flags & VKD3D_CONFIG_FLAG_SHADER_CACHE))
        return S_OK;

    path = getenv("VKD3D_SHADER_CACHE");
    if (!path)
    {
        /* By default, just use file in CWD. Not a great idea, but convenient for early testing. */
        path = "default";
    }
    ...
    if (path[0] == '/')
        snprintf(cache->read_path, sizeof(cache->read_path), "Z:\\%s.vkd3d-proton-cache", path + 1);
    else
        snprintf(cache->read_path, sizeof(cache->read_path), "%s.vkd3d-proton-cache", path);
